...
- **Focus guard + controlled self-activation for ZCode-as-source forwards.** When you're talking to ZCode, the foreground window is ZCode (not VS Code). The `OfficialUiBridge` focus guard (`assertForegroundWorkspaceWindow`) checks `GetForegroundWindow()` and rejects unsafe sends. For `agent-command` trigger only, Broker first tries one HWND-level activation of the already-identified current-workspace VS Code window (`ShowWindowAsync` + `AttachThreadInput` + `BringWindowToTop` + `SetForegroundWindow`; `AppActivate(pid)` only if HWND is unavailable), waits briefly, then **re-runs the same foreground guard** — only if re-check passes does it write clipboard / focus Codex/Claude / send. If re-check still fails, the send is rejected; the safety boundary is never bypassed. Foreground detection now reads hwnd + pid + processName + title (not just PID) to avoid misjudging Electron window ownership. Do NOT remove this guard or send after a failed re-check — it is the v0.0.3 fail-safe working as designed.
...
- ZCode -> Codex / ClaudeCode still requires the VS Code workspace window to be foreground because the official-panel SendKeys path is guarded. `agent-command` may perform one controlled HWND-level VS Code foreground activation attempt and then re-check the guard. If the guard still rejects, treat it as a foreground requirement, not MCP failure.
...
- Do not move the mouse pointer or add broad fallback branches that can open the wrong conversation. The only allowed background activation is the `agent-command` path's single HWND-level attempt against the uniquely identified current-workspace VS Code window, followed by the normal foreground guard.
